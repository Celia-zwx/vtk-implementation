...
public:
  ...
  {
    if (vtkCommand::TimerEvent == eventId)//为什么是==不是从属
    {
      ++this->TimerCount;
    }
    ...
    else
    {
      vtkRenderWindowInteractor* iren =
          dynamic_cast<vtkRenderWindowInteractor*>(caller);
      if (this->timerId > -1)//不清楚有什么作用
      {
        iren->DestroyTimer(this->timerId); //Destroy the timer specified by timerId.Returns 1 if the timer was destroyed.
      }
    }
  }
...
int main(int, char*[])
{
  ...
  int timerId = renderWindowInteractor->CreateRepeatingTimer(500);//时间间隔0.5s
